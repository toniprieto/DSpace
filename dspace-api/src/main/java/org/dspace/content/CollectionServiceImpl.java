...
//            items.remove();
    ...
    @Override
    public List<Collection> findCollectionsWithMap(Context context, String q, Community community,
                                                             int offset, int limit) throws SearchServiceException {
        ...
        discoverQuery.setMaxResults(limit);
        discoverQuery.setSortField(SOLR_SORT_FIELD, SORT_ORDER.asc);
        DiscoverResult resp = retrieveCollectionsWithMap(context, discoverQuery, null, community, q);
        for (IndexableObject solrCollections : resp.getIndexableObjects()) {
            Collection c = ((IndexableCollection) solrCollections).getIndexedObject();
            collections.add(c);
        }
    ...
    private DiscoverResult retrieveCollectionsWithMap(Context context, DiscoverQuery discoverQuery,
                                                         String entityType, Community community, String q)
        throws SearchServiceException {
