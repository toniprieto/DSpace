...
public class SolrServiceResourceRestrictionPlugin implements SolrServiceIndexPlugin, SolrServiceSearchPlugin {
    ...
    public void additionalSearchParameters(Context context, DiscoverQuery discoveryQuery, SolrQuery solrQuery,
        ...
        try {
            if (!authorizeService.isAdmin(context)) {
                ...
                /*
                 * The `actions` array specifies the permissions required beyond the default "read" permission.
                 ...
                 * - If "admin" is in the `actions` array, it checks only for admin permissions.
                 * - Otherwise, it checks for both "read" and the specified actions.
                 *
                 * The resulting query follows this structure: (read AND action) OR admin.
                 */
                if (actions.length == 0) {
                    // If no actions are passed, we only check for read permissions
                ...
                // Add the location query for include the objects that are in communities or collections that the user
                // administrate
                ...
                log.info("Time taken to create location query: " + (endTime - startTime) + " ms");
    ...
    /**
     * Get the action name used for solr indexing based on the action id
     *
