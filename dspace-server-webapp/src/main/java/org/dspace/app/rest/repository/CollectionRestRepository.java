...
public class CollectionRestRepository extends DSpaceObjectRestRepository<Collection, CollectionRest> {
    ...
    public Page<CollectionRest> findAdminAuthorized (
        ...
        @Parameter (value = "filterQuery") String filterQuery) {
        return findAuthorized(pageable, new int[] { Constants.ADMIN }, query, filterQuery);
    }
    ...
    public Page<CollectionRest> findEditAuthorized (
        Pageable pageable, @Parameter(value = "query") String query,
        @Parameter (value = "filterQuery") String filterQuery) {
        return findAuthorized(pageable, new int[] { Constants.WRITE }, query, filterQuery);
    }
     ...
     */
    @PreAuthorize("hasAuthority('AUTHENTICATED')")
    @SearchRestMethod(name = "findMapAuthorized")
    public Page<CollectionRest> findMapAuthorized(
        Pageable pageable, @Parameter(value = "query") String query,
        @Parameter (value = "filterQuery") String filterQuery) {
        return findAuthorized(pageable, new int[] {Constants.ADD, Constants.WRITE}, query, filterQuery);
    }

    private Page<CollectionRest> findAuthorized(Pageable pageable, int[] actions, String query, String filterQuery) {
        try {
            Context context = obtainContext();
            List<Collection> collections = authorizeService.findAuthorizedByActionCollection(context, query,
                filterQuery, actions,
                Math.toIntExact(pageable.getOffset()),
                Math.toIntExact(pageable.getPageSize()));
            long tot = authorizeService.countAuthorizedByActionCollection(context, query, filterQuery, actions);
            return converter.toRestPage(collections, pageable, tot, utils.obtainProjection());
